...
export const manageTaskTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      title: z.string().optional().nullable().describe('Title of the task (required for add)'),
      description: z.string().optional().nullable().describe('Description of the task'),
      priority: z.nativeEnum(Priority).optional().nullable().describe('1=LOW, 2=MEDIUM, 3=HIGH, 4=URGENT, 5=CRITICAL'),
      cognitiveLoad: z.nativeEnum(CognitiveLoad).optional().nullable().describe('1=LOW, 2=MEDIUM, 3=HIGH'),
      estimatedMinutes: z.number().positive().optional().nullable().describe('Estimated duration in minutes (default 30)'),
      dueDate: z.string().optional().nullable().describe('Due date in YYYY-MM-DD format, if applicable'),
      preferredTime: z.string().optional().nullable().describe('Preferred time of day (e.g. morning, afternoon, night, or specific time)'),
      isFixed: z.boolean().optional().nullable().describe('Whether this task MUST happen at a fixed time'),
      fixedStartTime: z.string().optional().nullable().describe('Fixed start time (HH:mm) if isFixed is true'),
      fixedEndTime: z.string().optional().nullable().describe('Fixed end time (HH:mm) if isFixed is true'),
      targetTaskName: z.string().optional().nullable().describe('The name of the existing task to delete, modify, complete, or skip. (Not needed for add)'),
    }),
  }
);
